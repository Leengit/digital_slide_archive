...
RUN cd /opt && \
    ...
    # git clone -b worker-broker-config-changes  https://github.com/girder/girder && \
    cd /opt/girder && \
    pip install --no-cache-dir -e .[mount] && \
    pip install --no-cache-dir -e clients/python && \
    cd girder/web && \
    # sed -i 's/"as-jqplot": "~1.0.8",/"updated-jqplot": "1.0.9-2",/g' package.json && \
    # sed -i 's/as-jqplot/updated-jqplot/g' src/views/body/AssetstoresView.js && \
    # npm install && \
    npm ci && \
    npx vite build --base=/ && \
...
RUN cd /opt && \
    ...
    # git config --global user.email 'you@example.com'; git config --global user.name 'Your Name'; git config --global pull.rebase false; git config --global core.editor vim && \
    # git merge --no-edit origin/convert-dicomweb-plugin-to-girder5 && \
    # git checkout convert-dicomweb-plugin-to-girder5 && \
...
RUN cd /opt && \
    ...
    cd /opt/HistomicsUI && \
    git checkout girder-5 && \
    # git checkout fix-importing-vue-component && \
    sed -i 's/==1.32.0a2//g' setup.py && \
...
# additional:
# clamav, annotation-tracker, renalpath, volview, dive
