...
RUN cd /opt/girder_worker && \
    git fetch --all && git checkout docker-run-options && \
    echo 'task_reject_on_worker_lost = True' >> /opt/girder_worker/girder_worker/celeryconfig.py && \
    echo 'task_acks_late = True' >> /opt/girder_worker/girder_worker/celeryconfig.py
